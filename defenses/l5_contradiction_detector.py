...
_SYSTEM = (
    ...
    "- разные численные значения одной и той же сущности по сути одной "
    "(A: «суточные 1500 руб», B: «суточные 5000 руб»; A: «лимит 100 000», "
    "B: «лимит снят»; A: «срок 5 лет», B: «срок 10 лет» по одному типу "
    "документа);\n"
    "- прямо противоположные правила (A: «запрещено», B: «разрешено без "
    "согласований»; A: «требует приказа», B: «согласование не требуется»);\n"
    "- противоположные действия (A: «нужно подавать заявление за 14 дней», "
    "B: «заявление не требуется»).\n\n"
    "ВАЖНО: документы разной даты, объявленные «новой редакцией», тоже "
    "противоречивы для одного запроса — пользователь хочет знать актуальную "
    "норму, а агент должен указать на конфликт. Помечай как противоречие.\n\n"
